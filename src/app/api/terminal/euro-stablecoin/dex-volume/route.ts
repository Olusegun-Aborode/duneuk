...
export async function GET() {
  try {
    const result = await getDuneQueryResults(EURO_QUERY_IDS.DEX_VOLUME);
    const data = { ...result, data: normalizeEurFields(result.data) };
    return NextResponse.json(data);
  } catch (error) {
    console.error("Failed to fetch euro DEX volume:", error);
    return NextResponse.json(
      { error: "Failed to fetch euro DEX volume" },
      { status: 500 }
    );
  }
}
